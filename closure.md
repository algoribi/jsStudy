...
내부의 f함수는 if 블록에 지역적으로 나타나기 때문에, test(true)를 호출하면 ["local", "global"]을 반환하고, test(false)를 호출하면 ["global"]을 반환할까?
아니면 함수 선언문은 앞서 **호이스팅**된다고 배웠기 때문에 지역 블록의 f함수로 오버라이딩되어 ["local", "local"]과 ["local"]을 반환할까?
